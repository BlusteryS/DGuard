...
class Forms {
	...
	public function f_control_list(Player $player): void {
		$regions = [];
	...
	public function f_edit_players(Player $player, string $region): void {
		DGuard::getInstance()->region[strtolower($player->getName())] = $region;
		...
		$form->setContent("Выберите нужного игрока для редактирования.\n" . "\n" . "Типы игроков:\n" . "§bЗеленые§f - Жители региона.\n" . "§bБирюзовые§f - Гости региона.");
		foreach ($tmp as $p) {
	...
	public function f_edit_role(Player $player, $p, $region): void {
		DGuard::getInstance()->region[strtolower($player->getName())] = $region;
	...
	public function f_remove(Player $player, $region): void {
		DGuard::getInstance()->region[strtolower($player->getName())] = $region;
		$form = new SimpleForm(function (Player $player, ?int $data) {
			if ($data !== NULL) {
				$region = DGuard::getInstance()->region[strtolower($player->getName())];
	...
	public function f_private(Player $player): void {
		$form = new SimpleForm(function (Player $player, ?int $data) {
			if ($data !== NULL) {
				switch ($data) {
					case 0:
	...
	public function f_create_region(Player $player): void {
		$form = new CustomForm(function (Player $player, ?array $data) {
			if (isset($data[1])) {
				if (strlen($data[1]) > 3 && strlen($data[1]) < 15) {
					...
					if ($result === 0) {
						...
						$player->sendMessage("§l§c> §eРегион с таким названием уже существует.§r");
					} elseif ($result === 2) {
						$player->sendMessage("§l§c> §eРегион пересекает другие регионы.§r");
					} elseif ($result === 3) {
						$player->sendMessage("§l§c> §eРегион занимает слишком огромную площадь.§r");
					} elseif ($result === 5) {
						$player->sendMessage("§l§c> §eВ данном мире нельзя создавать регионы.§r");
					} elseif ($result === 4) $player->sendMessage("§l§c> §eДостигнут лимит регионов.§r");
	...
	public function f_regions_list(Player $player): void {
	...
	public function f_regions_info(Player $player, $region): void {
		if ($region != "") {
